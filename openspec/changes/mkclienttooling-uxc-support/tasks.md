...
- [x] 6.5 Export via `flake.nix` so downstream repos can call `agentplot-kit.lib.mkUxcProjection`
...
- [x] 7.5 Register in `flake.nix` as a home-manager module export
...
- [x] 9.2 Add `README.md` section on UXC projection with the atomic-example snippet from `mockups/atomic.example.nix`
- [x] 9.3 Document the `op` service account prerequisite (users need `OP_SERVICE_ACCOUNT_TOKEN` in their shell env or an interactive `op signin`)
- [x] 9.4 Document the dual-secret pattern for migrations (legacy CLI + UXC) — link to `mockups/paperless.example.nix`

## 10. Wrapper-Skill Pipeline (D11–D15)

- [x] 10.1 Vendor upstream `uxc-skill-creator` at `skills/uxc-skill-creator/` (initial copy from holon-run/uxc); record source ref in a `VERSION` or sidecar file
- [x] 10.2 Add a flake input or git-submodule pin for the upstream skill so version bumps surface as PR diffs
- [x] 10.3 Implement `apps.<system>.author-skill` as a `pkgs.writeShellApplication`; wrap claude headless invocation with capability extraction (`nix eval` for host/linkName/authType/schemaSource) and output-dir resolution (`services/<svc>/skills/<protocol>/`)
- [x] 10.4 Recipe retry loop: capture validate.sh output on failure, re-prompt claude with errors as additional context, max 3 attempts
- [x] 10.5 Recipe writes `.source-spec-hash` sidecar after success — SHA-256 of spec content (file or fetched URL)
- [x] 10.6 Recipe early-exits with clear error if `ANTHROPIC_API_KEY` is unset
- [x] 10.7 Add `nix flake check` derivation that iterates over committed wrapper skills, runs each `scripts/validate.sh`, fails on any non-zero exit
- [x] 10.8 Add `nix flake check` derivation that recomputes `sha256(spec)` for each service, compares against `.source-spec-hash` sidecar, fails on mismatch
...
- [x] 10.1 Add a test that a service declaring only `openapi` (no `mcp`) evaluates correctly with `uxc.enabled = true` and a client with `uxc.enabled = false`
- [x] 10.2 Add a test that a service with both `mcp` and `openapi` referencing the same `auth.secret` produces ONE credential and TWO bindings
- [x] 10.3 Add a test that an `auth.secret` referencing a missing secret name fails eval with a clear error
